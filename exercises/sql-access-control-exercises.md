...
In the tasks, you will be granting privileges to another user to your database objects and observe how those privileges affect the database operations. So, the tasks require you to use two users.

If you are using Haaga-Helia's SQL Server (non-Windows users)_, all you need to do to get started is to log in using the SQL Server Management Studio. Take a look at the [Using Haaga-Helia's SQL Server](../materials/sql-server/haaga-helia-sql-server.md) guide if you have trouble.
...
3. In the Windows app bar, type "Services" in the search bar and click the "Services" application. Scroll down to the "SQL Server" service, right-click it, and click "Restart".
...
3. Next, let's log in using another user without any privileges. Open a new connection to your SQL Server instance by clicking "Connect" in the "Object Explorer" panel on the left and choosing "Database Engine". Choose the "Authentication" option as "SQL Server Authentication" and log in with the following username and password:
  ...
  - In _your own computer's SQL Server_, the username and password are the ones you created previously (username `visitor` and password `SECRET_password_12345` by default).
...
1. In _your user's query window_, create a new database role depending on your SQL Server setup:
  - In _Haaga-Helia's SQL Server_, create a role named `friends_of_XX` and replace `XX` with the last two digits from your Haaga-Helia's SQL Server username.
