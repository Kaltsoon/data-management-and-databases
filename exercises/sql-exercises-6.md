...
> [!TIP]
> Especially with the join operations, we need to understand the relationships between the tables. Use the [database diagram](https://drawsql.app/teams/kaltsoon/diagrams/takkula-database-diagram) to understand the primary and foreign key relationships of the tables.
...
> [!TIP]
> The name of the foreign key column doesn't need to be the same as the name of the primary key column. For example, `captain_id` foreign key column in a `Team` table could be referencing the `player_id` primary key column in the `Player` table. Use the Database Diagram in this week's "Materials" section on Moodle's "Course outline" page to identify the the primary and foreign keys.
